...
export const buildSystemPrompt = (
  ...
  lines.push(
    '5. Шаблон: { "$template": "Hello, ${/some/path}" } - интерполирует значение в строку. Абсолютные пути типа `${/some/path}` получают значение из стейта. Относительные (чистые) пути типа `${some/path}` получают значение из элемента массива (внутри repeat).'
  );
